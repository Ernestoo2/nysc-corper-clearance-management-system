...
export const listByCorper = query({
  args: { corperId: v.id("corpers"), limit: v.optional(v.number()) },
  handler: async (ctx, args) => {
    await requireAdmin(ctx);
    const limit = Math.min(200, Math.max(1, args.limit ?? 50));
    return await ctx.db
      .query("corperAuditLogs")
      .withIndex("by_corper_id_and_timestamp", (q) => q.eq("corperId", args.corperId))
      .order("desc")
      .take(limit);
  },
});
